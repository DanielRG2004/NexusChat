...
exports.health = async (req, res) => {
  res.json({ ok: true, message: 'Email auth funcionando' });
};

// ===============================
// REQUEST CODE (SIN CAMBIOS)
// ===============================
...
// ===============================
// VERIFY CODE (SIN CAMBIOS)
// ===============================
exports.verifyCode = async (req, res, next) => {
  try {
    const { tempToken, codigo } = req.body;
    if (!tempToken) return res.status(400).json({ ok: false, message: 'Falta el token temporal' });
    ...
    try {
      ...
      return res.status(401).json({ ok: false, message: 'Token temporal invalido o expirado' });
    }
    ...
    if (!user) return res.status(404).json({ ok: false, message: 'No se encontro el usuario pendiente' });
    ...
    await pool.query(`UPDATE verificaciones SET estado = 'usado', verificado_at = NOW() WHERE id = ?`, [verificationRow.id]);
    return res.json({
      ok: true,
...
// ===============================
// COMPLETE REGISTRATION (CORREGIDO isAdmin)
// ===============================
exports.completeRegistration = async (req, res, next) => {
  try {
    const { verificationToken } = req.body;
    if (!verificationToken) return res.status(400).json({ ok: false, message: 'Falta el token de verificacion' });

    let payload;
    try {
      payload = jwt.verify(verificationToken, JWT_SECRET);
    } catch {
      return res.status(401).json({ ok: false, message: 'Token de verificacion invalido o expirado' });
    }
    if (payload.purpose !== 'finish-registration') return res.status(401).json({ ok: false, message: 'Token de verificacion invalido' });
    ...
    if (user.estado_cuenta !== 'verificado') {
      await pool.query(
        `UPDATE usuarios SET estado_cuenta = 'verificado', email_verificado_at = NOW(), last_login_at = NOW() WHERE id = ?`,
        [user.id]
      );
    } else {
      await pool.query(`UPDATE usuarios SET last_login_at = NOW() WHERE id = ?`, [user.id]);
    }
    ...
    return res.json({
      ...
      token,
      isAdmin: isAdmin(updatedUser.email),
...
// ===============================
// LOGIN (CORREGIDO isAdmin)
// ===============================
exports.login = async (req, res) => {
  try {
    const { email, password } = req.body;
    if (!email || !password) return res.status(400).json({ ok: false, message: 'Correo y contraseña son obligatorios' });

    const cleanEmail = normalizeEmail(email);
    const [rows] = await pool.query(`SELECT * FROM usuarios WHERE LOWER(email) = LOWER(?) LIMIT 1`, [cleanEmail]);
    const user = rows[0];
    if (!user) return res.status(404).json({ ok: false, message: 'Usuario no encontrado' });
    if (user.estado_cuenta === 'bloqueado') return res.status(403).json({ ok: false, message: 'Tu cuenta esta bloqueada' });
    if (user.estado_cuenta !== 'verificado') return res.status(403).json({ ok: false, message: 'Debes verificar tu correo antes de iniciar sesion' });

    const validPassword = await bcrypt.compare(password, user.password);
    if (!validPassword) return res.status(401).json({ ok: false, message: 'Credenciales incorrectas' });
    ...
    return res.status(500).json({ ok: false, message: 'Error al iniciar sesion', details: error.message });
  }
};
...
exports.getMe = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT id, nombre, email, telefono, foto_perfil, descripcion, estado_cuenta, email_verificado_at, last_login_at FROM usuarios WHERE id = ?`,
      [userId]
    );
...
// ===============================
// UPDATE PROFILE (CORREGIDO: telefono estaba sin declarar en los parámetros)
// ===============================
exports.updateProfile = async (req, res) => {
  try {
    ...
    if (nombre !== undefined) {
      ...
      if (cleanNombre.length < 3) return res.status(400).json({ ok: false, message: 'El nombre debe tener al menos 3 caracteres' });
      updates.push('nombre = ?');
    ...
    if (updates.length === 0) return res.status(400).json({ ok: false, message: 'No hay campos para actualizar' });
